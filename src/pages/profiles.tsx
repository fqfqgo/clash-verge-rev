  ...
  const onImport = async () => {
    ...
    try {
      ...
      while (true) {
        ...
        if (String(result.err).toLowerCase().includes('legacy tls')) {
          showNotice.error(String(result.err))
          break
        }
        ...
        try {
          ...
          await handleImportSuccess(
            'profiles.page.feedback.notifications.importWithClashProxy',
          )
        } catch (retryErr) {
          if (isSubscriptionPasswordError(retryErr)) {
            let retryOption = option
            let lastPwErr: unknown = retryErr
            while (true) {
              const password = await promptSubscriptionPassword(
                isSubscriptionWrongPassword(lastPwErr),
